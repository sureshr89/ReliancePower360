from __future__ import annotations
import json
from datetime import datetime, timezone
from pathlib import Path
import pandas as pd
from collectors import collect_all
from intelligence import analyse, deduplicate, outlook
from price_tracker import fetch_reliance_power_price
from news_price_analysis import explain_price_news_relation
from forecast_engine import build_explanations, forecast
from forecast_audit import record_prediction, audit_eod
from data_store import append_history

DATA=Path("data"); DATA.mkdir(exist_ok=True)

def main():
    raw=collect_all()
    analysed=deduplicate(analyse(raw))
    if not analysed.empty:
        analysed=analysed.sort_values(["impact","source_reliability","relevance"],ascending=False)
    analysed.to_csv(DATA/"latest_news.csv",index=False)

    base=outlook(analysed)
    price=fetch_reliance_power_price()
    relation=explain_price_news_relation(analysed,price)
    explanations=build_explanations(analysed,price)
    forecasts=forecast(analysed,price,base)
    generated=datetime.now(timezone.utc).isoformat()

    summary={"news_score":base["score"],"news_outlook":base["outlook"],"confidence":base["confidence"],
      "article_count":int(len(analysed)),
      "bullish_count":int((analysed["sentiment"]=="BULLISH").sum()) if not analysed.empty else 0,
      "bearish_count":int((analysed["sentiment"]=="BEARISH").sum()) if not analysed.empty else 0,
      "neutral_count":int((analysed["sentiment"]=="NEUTRAL").sum()) if not analysed.empty else 0}

    report={"generated_at":generated,"company":"Reliance Power","model_version":"2.0-today-yesterday-eod",
      "summary":summary,"price":price,"news_price_relation":relation,"today_explanation":explanations,
      "forecast":forecasts,"prediction_news_window":forecasts.get("news_window",{}),
      "source_breakdown":analysed.groupby("source_type").size().to_dict() if not analysed.empty else {}}
    (DATA/"latest_report.json").write_text(json.dumps(report,indent=2,ensure_ascii=False),encoding="utf-8")

    pred=forecasts.get("current_session",{})
    record_prediction(generated,pred,price.get("last_price"))
    audit_eod(price.get("last_price"))

    append_history({"generated_at":generated,"score":base["score"],"outlook":base["outlook"],
      "articles":summary["article_count"],"price":price.get("last_price"),
      "daily_price_change_pct":price.get("daily_change_pct"),"prediction":pred.get("outlook"),"prediction_score":pred.get("score")})
    print(json.dumps({"articles":summary["article_count"],"price":price,"outlook":summary["news_outlook"],"prediction":pred},indent=2))

if __name__=="__main__":
    main()
